    ...
    public class KitchenController : ControllerAuthorizeBase
    {
        ...
        public ActionResult KOT(string searchby1, string search1, string searchby2, string search2, string searchby3, string search3, string searchby4, string search4, string sortby, int? page, string status)
        {
            ...
            var kot = (from oi in kitchenServices.OrderItems.GetByAll().Where(x => x.Active == true).ToList()
                       join f in kitchenServices.FoodItems.GetByAll().Where(x => x.Active == true).ToList() 
                       on oi.FoodItemID equals f.FoodItemID
                       join k in kitchenServices.Kitchen.GetByAll().Where(x => x.Active == true).ToList() 
                       on f.KitchenID equals k.KitchenID
                       join om in kitchenServices.OrderMaster.GetByAll().Where(x => x.Active == true && (x.OrderStatus == "ConfirmOrder" && x.OrderStatus != "ReadyPickup")  && x.IsBillPaid == false).ToList() 
                       on oi.OrderMasterID equals om.OrderMasterID                     
